 ...
 *   http://immortal.pl/
 * Copyright 2013 Departement du Rhone
 *   <support@erasme.org>
 * 
...
namespace NReadability
{
	...
	public class NReadabilityTranscoder
	{
		...
		public XDocument LoadDocument(string htmlContent)
		{
			return SgmlDomBuilder.BuildDocument(htmlContent);
		}

		public void GetParts(XDocument document, string url, out XElement articleTitleElement, out XElement articleContentElement)
		{
			...
			articleTitleElement = ExtractArticleTitle(document);
			articleContentElement = ExtractArticleContent(document);
		}
    ...
    internal string FindBaseUrl(string url)
    {
      ...
      return string.Format("{0}://{1}{2}", protocol, hostname, String.Join("/", cleanedSegments.ToArray()));
		...
		public void PrepareDocument(XDocument document)
		{
			...
			// remove all on* attribute
			foreach(XElement el in rootElement.DescendantsAndSelf()) {
				List<XAttribute> removeList = new List<XAttribute>();
				foreach(XAttribute attribute in el.Attributes()) {
					if(attribute.Name.LocalName.StartsWith("on", StringComparison.CurrentCultureIgnoreCase))
						removeList.Add(attribute);
				}
				foreach(XAttribute attribute in removeList)
					attribute.Remove();
			}
		...
		public XElement ExtractArticleTitle(XDocument document)
		{
			var documentBody = GetOrCreateBody(document);
		...
		public XElement ExtractArticleContent(XDocument document)
		{
    ...
    internal void CleanConditionally(XElement rootElement, string elementName)
    {
      ...
      RemoveElements(elementsToRemove);
		...
		public static void RemoveElements(IEnumerable<XElement> elementsToRemove)
		{
			elementsToRemove.ForEach(elementToRemove => elementToRemove.Remove());
		}

		public static void ResolveElementsUrls(XDocument document, string tagName, string attributeName, string url)
		{
			if(document == null)
