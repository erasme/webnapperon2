...
Ui.DropBox.extend('Wn.SFlowDropBox', {
	flow: undefined,
	fixed: undefined,
	marker: undefined,
	...
	constructor: function(config) {
		...
		Wn.SFlowDropBox.base.append.call(this, this.fixed);

		this.marker = new Ui.Frame({ margin: 2, frameWidth: 2, radius: 2, width: 6, height: 6 });
		this.marker.hide();
		this.fixed.append(this.marker);

		this.flow = new Ui.SFlow({ uniform: true, itemAlign: 'stretch' });
		Wn.SFlowDropBox.base.append.call(this, this.flow);
	...
	setMarkerPos: function(pos) {
		...
		if(pos < this.flow.getChildren().length) {
			...
			var x = child.getLayoutX() - child.getMarginLeft() -
				(this.marker.getLayoutWidth() + this.marker.getMarginLeft() + this.marker.getMarginRight() + this.flow.getSpacing())/2;
			...
			var x = child.getLayoutX() + child.getLayoutWidth() - (this.marker.getLayoutWidth() + this.flow.getSpacing())/2;
			var y = child.getLayoutY();
			var height = child.getLayoutHeight();
			this.marker.setHeight(height);
			this.fixed.setPosition(this.marker, x, y);
...
//		console.log('onBoxDrop mimetype: '+mimetype+', data: '+data+' insertAt: '+this.findPosition({x: x, y: y }));
		...
		var position = this.findPosition({x: x, y: y });
		if(this.checkPosition(position))
			this.fireEvent('dropat', this, mimetype, data, position);
	},
